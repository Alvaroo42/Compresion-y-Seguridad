...
public class Main {
    public static void main(String[] args) {
        try {
            ...
            System.out.println(
                    "Clave AES Cifrada con RSA (Base64): " + Base64.getEncoder().encodeToString(claveAESCifrada));

            // C) Comprobación: Desciframos con la clave Privada para asegurar que
            // recuperamos la misma AES
            byte[] claveAESRecuperada = CifradoRSA.descifrarClaveAES(claveAESCifrada, parClavesUsuario.getPrivate());
            System.out.println(
                    "Clave AES Recuperada (Base64): " + Base64.getEncoder().encodeToString(claveAESRecuperada));
            ...
            System.out.println("Clave derivada de password (Base64): "
                    + Base64.getEncoder().encodeToString(claveAESDesdePassword));
            ...
            System.out.println("\n--- 4. GUARDADO EN DISCO ---");
            // Extraemos los bytes puros de la clave pública generada en la Parte 2
            byte[] clavePublicaEnBytes = parClavesUsuario.getPublic().getEncoded();

            // Llamamos a nuestro nuevo gestor
            GestorFicheros.guardarClavesUsuario("usuario_prueba", clavePublicaEnBytes, clavePrivadaCifrada);
